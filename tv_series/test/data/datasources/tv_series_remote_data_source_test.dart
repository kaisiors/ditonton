...
void main() {
  ...
  group('get tv series detail', () {
    ...
    test(
        'should return Server Exception when the response code is 404 or other',
        () async {
      // arrange
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/$tId?$API_KEY')))
          .thenAnswer((_) async => http.Response('Not Found', 404));
  ...
  group('search TV Series', () {
    final tSearchResult = TvSeriesResponse.fromJson(
            json.decode(readJson('dummy_data/search_dragon_tv_series.json')))
        .tvSeriesList;
    const tQuery = 'Dragon';
    ...
    test('should return list of TV Series Model when the response code is 200',
      ...
      when(mockHttpClient
          ...
          .thenAnswer((_) async => http.Response(
              readJson('dummy_data/search_dragon_tv_series.json'), 200));
      // act
  ...
  group('get TV Series episode', () {
    ...
    test('should return list of Episode Model when the response code is 200',
      ...
      // act
      final result = await dataSource.getEpisodeTvSeries(tId, tSeasonNumber);
      // assert
